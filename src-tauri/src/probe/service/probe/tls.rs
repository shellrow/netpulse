...
use crate::probe::service::probe::{ProbeContext, PortProbeResult};
    ...
    if let Some(cert) = client_conn.peer_certificates().and_then(|v| v.first()).cloned() {
        ...
        match X509Certificate::from_der(&cert) {
            Ok((_, x509)) => {
                ...
                let sig_alg_name = db::tls::oid_sig_name(x509.signature_algorithm.oid().to_id_string().as_str());
                tls_info.sig_algorithm = Some(sig_alg_name);
                let pubkey_alg_name = db::tls::oid_pubkey_name(x509.public_key().algorithm.oid().to_id_string().as_str());
